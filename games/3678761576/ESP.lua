-- ESP.lua — games/3678761576 (WeaponModule game) — sUNC
-- Player ESP: box + health bar + name/distance + icon, scaled by projected
-- geometry, behind-camera targets fully hidden (Z check).
--   Team Check ON  -> teammates hidden entirely (Roblox Teams — this game
--                     demonstrably uses them: WeaponModule checks Player.Team)
--   Team Color ON  -> box/text take the player's TeamColor
--   Visibility ON  -> camera has line of sight = GREEN, blocked = RED
-- (visibility beats team colors while enabled, same as the other ESPs)
--
-- This game's WeaponModule references Team/TeamColor everywhere, and its own
-- bulletMagnetism even checks a child "Team" StringValue on NPCs — the Roblox
-- Teams path should work out of the box.
...
-- ======================== icon rendering =============================
-- ImageLabels in a hidden ScreenGui (Drawing Images don't rasterize on
-- Potassium — proven lesson from the Decay ESP).
local iconGui = nil
...
local function renderSet(set, box, dist, label, color, hp, iconId)
	...
	-- icon above the name
	if set.icon then
		if Config.ShowIcons and iconId and box then
			pcall(function()
				...
				-- lucide masks take the ESP color
				...
				if root and hum and hum.Health > 0 then
					...
					if shouldShow(dist) then
						...
						if box then
							...
							local Icons = Shrimp.Modules.Icons
							local iconId = Icons and Icons.Get and Icons.Get(Config.PlayerIcon) or nil
